...
public class PokerTableController implements Initializable {
	...
	public void btnDrawCard_Click(ActionEvent event) {
		...
		for (int iPosition = 1; iPosition < 3; iPosition++) {
			...
			pt.getElements().add(new MoveTo(pntDeck.getX(),pntDeck.getY() - 10));
		
			ArcTo arcto = new ArcTo();
			arcto.setX(pntCardDealt.getX());
			arcto.setY(pntCardDealt.getY());
			arcto.setRadiusX(30);
			arcto.setRadiusY(30);
			
			pt.getElements().add(arcto);
			...
			patTMoveRot.getChildren().addAll(pathT);
